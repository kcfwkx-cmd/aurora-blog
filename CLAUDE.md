...
- 单个 session 超过 500KB 时，主动提醒我使用 /compact 压缩或新开对话
- 单个 session 超过 1MB 时，必须提醒我新开 session，此时上下文质量已下降
- 每个 session 建议按功能模块命名，例如：评论系统、搜索功能、侧边栏组件等
- 当发现自己开始重复之前说过的内容时，主动提醒我上下文过长